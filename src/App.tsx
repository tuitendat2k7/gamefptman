...
const SkyBackground = ({ slot, phase }: { slot: string, phase: string }) => {
  ...
  const activeSlot = isGameplay ? slot : "evening"; // Các màn hình hệ thống mặc định là ban đêm
  ...
  return (
    <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none transition-colors duration-1000">
      ...
      {/* Ngôi Sao Lấp Lánh (Chỉ hiện ban đêm) */}
      <AnimatePresence>
        {activeSlot === 'evening' && (
  ...
  return (
    <div className={`space-y-1.5 ${pulse ? "animate-pulse" : ""}`}>
      ...
      <div className="h-4.5 w-full bg-zinc-950/80 rounded-full overflow-hidden border border-zinc-900 shadow-inner relative">
        ...
        {pulse && <div className="absolute inset-0 bg-red-500/10 animate-ping rounded-full pointer-events-none" />}
      </div>
    </div>
  );
};
...
const CharacterAvatar: React.FC<any> = ({ stats, major }) => {
  let face = "🧑‍🎓"; let moodName = "Ổn định"; let bubbleText = "Học kỳ năm nhất thật thú vị, cố gắng sống sót thôi!"; let glowColor = "shadow-[0_0_40px_rgba(139,92,246,0.15)] border-violet-500/20 bg-violet-500/10"; let characterAnimation = "animate-pulse";
  if (stats.stress >= 80) { face = "🤯"; moodName = "Quá tải"; bubbleText = "Áp lực điên cuồng, đầu sắp bốc hỏa rồi! Phải đi xả hơi gấp!"; glowColor = "shadow-[0_0_40px_rgba(239,68,68,0.25)] border-red-500/30 bg-red-500/10"; characterAnimation = "animate-bounce"; }
  else if (stats.energy <= 20) { face = "😪"; moodName = "Lao lực"; bubbleText = "Hết sạch pin rồi... Mắt díp lại... Cần ngủ gấp..."; glowColor = "shadow-[0_0_40px_rgba(107,114,128,0.2)] border-zinc-500/30 bg-zinc-500/10"; characterAnimation = "animate-pulse"; }
  else if (stats.money <= 15) { face = "😭"; moodName = "Cháy túi"; bubbleText = "Hết tiền rồi! Có ai cứu đói bữa mì tôm không..."; glowColor = "shadow-[0_0_40px_rgba(245,158,11,0.2)] border-orange-500/30 bg-orange-500/10"; characterAnimation = "animate-bounce"; }
  else if (stats.gpa <= 30) { face = "🤡"; moodName = "Nguy kịch"; bubbleText = "GPA tụt dốc thảm hại, có khi nào phải bảo lưu về quê trồng rau..."; glowColor = "shadow-[0_0_40px_rgba(244,63,94,0.2)] border-rose-500/30 bg-rose-500/10"; }
  else if (stats.money >= 80) { face = "😎"; moodName = "Phú hộ"; bubbleText = "Rủng rỉnh tiền bạc! Chiều nay bao cả đám trà sữa lẩu nướng nhé!"; glowColor = "shadow-[0_0_40px_rgba(234,179,8,0.25)] border-yellow-500/30 bg-yellow-500/10"; }
  else if (stats.gpa >= 90) { face = "🤓"; moodName = "Học bá"; bubbleText = "Kiến thức lấp lánh! Mấy đề kiểm tra này chỉ là muỗi thôi."; glowColor = "shadow-[0_0_40px_rgba(59,130,246,0.25)] border-blue-500/30 bg-blue-500/10"; }
  else if (stats.happiness >= 85) { face = "🥰"; moodName = "Yêu đời"; bubbleText = "Đời sinh viên vui ghê! Mọi thứ đang tiến triển cực tốt."; glowColor = "shadow-[0_0_40px_rgba(236,72,153,0.25)] border-pink-500/30 bg-pink-500/10"; }

  return (
    <div className={`p-5 rounded-2xl border transition-all duration-500 flex flex-col items-center backdrop-blur-md ${glowColor}`}>
      <span className="text-xs text-zinc-300 font-black tracking-widest uppercase mb-3.5">Tình Trạng Nhân Vật</span>
      <div className="relative flex items-center justify-center h-24 w-24 bg-zinc-950/80 rounded-full border border-zinc-800 shadow-inner">
        <span className={`text-5xl select-none ${characterAnimation}`}>{face}</span>
      </div>
      <div className="mt-2.5 text-center">
        <span className="px-3 py-1 rounded-full bg-zinc-950/80 border border-zinc-800 text-[11px] font-mono font-black text-stone-100 uppercase tracking-widest">Trạng thái: {moodName}</span>
      </div>
      <div className="mt-4 w-full relative bg-zinc-950/80 border border-zinc-800/80 p-3 rounded-xl text-center">
...
export default function App() {
  ...
  const unlockAchievement = (id: string) => {
    if (!unlockedAchievements.includes(id) && currentUser) {
      ...
      // Lưu cúp gắn liền với username của người chơi
      localStorage.setItem(`achievements_${currentUser.username}`, JSON.stringify(updated));
  ...
  return (
    <div className="min-h-screen text-stone-100 font-sans selection:bg-amber-500 selection:text-black flex flex-col justify-between relative overflow-hidden">
      ...
      <AnimatePresence>
        {unlockedNotice && (
          <motion.div initial={{ opacity: 0, y: -50, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -20, scale: 0.9 }} className="fixed top-6 left-1/2 -translate-x-1/2 z-[100] px-5 py-3 rounded-2xl bg-zinc-950/90 border border-amber-500/30 text-amber-300 font-bold text-xs shadow-[0_10px_30px_rgba(245,158,11,0.15)] flex items-center gap-2.5 backdrop-blur-md">
      ...
      <header className="border-b border-zinc-800/50 bg-[#070709]/60 backdrop-blur-xl sticky top-0 z-35 py-3 px-4 md:px-6 shadow-sm">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {currentUser && phase !== "AUTH" && (
              <div className="flex items-center gap-2 bg-zinc-950/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-zinc-800">
            ...
            {phase === "GAMEPLAY" && (
              <button onClick={() => { gameAudio.playTap(); setShowHistoryModal(true); }} className="px-3 py-1.5 rounded-xl bg-zinc-950/60 backdrop-blur-md hover:bg-zinc-800/80 border border-zinc-800 transition-all text-xs font-bold text-stone-200 flex items-center gap-1.5 cursor-pointer select-none">
                <FileText className="h-3.5 w-3.5" />
                <span>Nhật ký ({history.length})</span>
              </button>
            )}
            <button onClick={() => { setSoundEnabled(!soundEnabled); gameAudio.playTap(); }} className="p-2 rounded-xl bg-zinc-950/60 backdrop-blur-md hover:bg-zinc-800/80 border border-zinc-800 text-stone-300 hover:text-stone-100 transition-all active:scale-90 cursor-pointer" title={soundEnabled ? "Tắt âm thanh" : "Bật âm thanh"}>
        ...
        <motion.div animate={isVibrating ? { x: [-6, 6, -6, 6, -3, 3, 0], y: [-2, 2, -2, 2, 0, 0, 0] } : {}} transition={{ duration: 0.4 }} className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          {phase !== "START" && phase !== "INTRO" && phase !== "AUTH" && phase !== "LEADERBOARD" && (
            <aside className="lg:col-span-4 space-y-5">
              <div className="p-5 rounded-3xl bg-[#0a0a0c]/80 backdrop-blur-xl border border-zinc-800/60 shadow-2xl space-y-5">
                <div className="flex items-center gap-3.5">
                  <div className="h-11 w-11 rounded-xl bg-zinc-950/80 border border-zinc-800 flex items-center justify-center text-xl shadow-inner shrink-0">{currentUser?.avatar || "🧑‍🎓"}</div>
              ...
              {currentWeather && (
                <div className={`p-5 rounded-2xl border backdrop-blur-md ${currentWeather.color} transition-all duration-300`}>
                  <div className="flex items-center gap-2.5 mb-2.5"><span className="text-2xl shrink-0">{currentWeather.icon}</span><h4 className="font-display font-black text-sm uppercase tracking-wider">{currentWeather.name}</h4></div>
                  <p className="text-xs md:text-sm text-stone-200 leading-relaxed font-bold font-sans">{currentWeather.description}</p>
                </div>
              )}
              <div className="p-5 rounded-2xl bg-[#0a0a0c]/80 backdrop-blur-xl border border-zinc-800/60 shadow-md">
                <div className="flex items-center gap-2 border-b border-zinc-800/50 pb-3 mb-4"><Award className="h-5 w-5 text-amber-500" /><h4 className="font-display font-black text-sm uppercase tracking-wider">Phòng Thành Tích ({unlockedAchievements.length}/6)</h4></div>
                <div className="grid grid-cols-7 gap-2">
                  {ACHIEVEMENTS.map(a => {
                    const isUnlocked = unlockedAchievements.includes(a.id);
                    return (
                      <div key={a.id} className={`relative h-11 rounded-xl flex items-center justify-center text-2xl border transition-all group ${isUnlocked ? "bg-amber-500/10 border-amber-500/30 text-amber-400 cursor-help" : "bg-neutral-900/40 border-zinc-800 text-zinc-600 opacity-60"}`} title={`${a.title}: ${a.description}`}>
                        <span>{a.icon}</span>
                        <div className="absolute bottom-13 left-1/2 -translate-x-1/2 z-50 w-52 p-3 rounded-xl bg-zinc-950 border border-zinc-800 text-xs text-stone-200 leading-snug hidden group-hover:block shadow-2xl pointer-events-none">
                          <p className="font-black text-stone-50 mb-1">{a.title}</p><p className="font-semibold text-zinc-400 mb-1.5">{a.description}</p><span className="text-[10px] bg-zinc-900 text-amber-400 px-1.5 py-0.5 rounded font-mono font-black">ĐK: {a.requirement}</span>
                        </div>
                      </div>
                    );
                  })}
          ...
          <div className={`${phase === "START" || phase === "INTRO" || phase === "AUTH" || phase === "LEADERBOARD" ? "lg:col-span-12" : "lg:col-span-8"} w-full`}>
            <AnimatePresence mode="wait">
              ...
              {phase === "AUTH" && (
                <motion.div key="auth" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, y: -20 }} className="max-w-md mx-auto w-full p-8 rounded-3xl bg-[#0a0a0c]/80 backdrop-blur-2xl border border-zinc-800/60 shadow-2xl space-y-6 relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-amber-500 via-rose-500 to-purple-500" />
                  
                  <div className="text-center space-y-2 mb-2">
                    <h2 className="text-2xl md:text-3xl font-display font-black uppercase tracking-widest text-transparent bg-clip-text bg-gradient-to-br from-stone-100 to-zinc-400">
                  ...
                  <div className="space-y-4">
                    <div className="relative">
                      ...
                      <input type="text" placeholder="Tên đăng nhập" value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} className="w-full bg-zinc-950/60 border border-zinc-700/50 focus:border-amber-500 rounded-xl py-3.5 pl-12 pr-4 text-sm font-bold outline-none transition-all" />
                    </div>
                    <div className="relative">
                      <KeyRound className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-zinc-400" />
                      <input type="password" placeholder="Mật khẩu" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} className="w-full bg-zinc-950/60 border border-zinc-700/50 focus:border-amber-500 rounded-xl py-3.5 pl-12 pr-4 text-sm font-bold outline-none transition-all" />
                    </div>
                    ...
                    {authMode === "register" && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="space-y-4">
                        <div className="relative">
                          ...
                          <input type="text" placeholder="Họ và Tên (VD: Nguyễn Văn A)" value={formData.fullName} onChange={e => setFormData({...formData, fullName: e.target.value})} className="w-full bg-zinc-950/60 border border-zinc-700/50 focus:border-amber-500 rounded-xl py-3.5 pl-12 pr-4 text-sm font-bold outline-none" />
                        </div>
                        <div className="relative">
                          <Hash className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-zinc-400" />
                          <input type="text" placeholder="Mã Số Sinh Viên (VD: DE210164)" value={formData.studentId} onChange={e => setFormData({...formData, studentId: e.target.value})} className="w-full bg-zinc-950/60 border border-zinc-700/50 focus:border-amber-500 rounded-xl py-3.5 pl-12 pr-4 text-sm font-bold outline-none uppercase" />
                        </div>
                        <div className="pt-2">
                          <label className="text-xs font-black text-zinc-300 uppercase tracking-widest block mb-2">Chọn Avatar:</label>
                          <div className="flex flex-wrap gap-2 justify-center">
                            {AVATARS.map(ava => (
                              <button key={ava} onClick={() => setFormData({...formData, avatar: ava})} className={`text-2xl p-2 rounded-xl border transition-all ${formData.avatar === ava ? "bg-amber-500/20 border-amber-500 scale-110" : "bg-zinc-950/60 border-zinc-800 grayscale opacity-60 hover:grayscale-0 hover:opacity-100"}`}>{ava}</button>
                            ))}
                          </div>
                        </div>
                      </motion.div>
                    ...
                    <p className="text-center text-xs font-medium text-zinc-300 cursor-pointer hover:text-amber-400" onClick={() => setAuthMode(authMode === "login" ? "register" : "login")}>
              ...
              {phase === "LEADERBOARD" && (
                <motion.div key="leaderboard" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto w-full p-8 rounded-3xl bg-[#0a0a0c]/80 backdrop-blur-2xl border border-zinc-800/60 shadow-2xl space-y-6">
                  ...
                  <div className="space-y-3 mt-6">
                    {leaderboard.length === 0 ? (
                      ...
                      leaderboard.slice(0, 10).map((entry, index) => (
                        <div key={index} className={`flex items-center justify-between p-4 rounded-2xl border backdrop-blur-sm ${index === 0 ? 'bg-amber-500/15 border-amber-500/40' : index === 1 ? 'bg-zinc-300/15 border-zinc-300/40' : index === 2 ? 'bg-orange-700/15 border-orange-700/40' : 'bg-zinc-950/60 border-zinc-800'}`}>
                          <div className="flex items-center gap-4">
                            <div className={`text-xl font-black ${index === 0 ? 'text-amber-400' : index === 1 ? 'text-zinc-200' : index === 2 ? 'text-orange-500' : 'text-zinc-400'}`}>#{index + 1}</div>
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-[10px] text-zinc-400 font-bold uppercase">Điểm tổng hợp</p>
                          </div>
                        </div>
                      ))
                    )}
                  </div>
                  <button onClick={() => setPhase(currentUser ? "START" : "AUTH")} className="w-full mt-6 py-4 rounded-xl bg-zinc-900/80 hover:bg-zinc-800 border border-zinc-700 text-stone-200 font-black text-sm uppercase transition-all cursor-pointer">
                    Quay lại
                  </button>
                </motion.div>
              )}
              ...
              {phase === "START" && (
                <motion.div key="start" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="max-w-xl mx-auto p-10 rounded-3xl bg-[#0a0a0c]/80 backdrop-blur-2xl border border-zinc-800/60 shadow-2xl text-center space-y-7">
                  <div className="relative inline-block mx-auto">
                    <span className="absolute inset-0 rounded-full bg-amber-500/10 blur-xl animate-pulse"></span>
                    <div className="h-24 w-24 rounded-2xl bg-zinc-950/80 border border-amber-500/30 flex items-center justify-center shadow-lg ring-1 ring-amber-500/10">
                      <GraduationCap className="h-12 w-12 text-amber-400 stroke-[1.2]" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <h1 className="text-4xl md:text-5xl font-display font-black tracking-widest bg-gradient-to-r from-amber-100 via-amber-200 to-amber-500 bg-clip-text text-transparent uppercase">Freshman Survival</h1>
                    <p className="text-stone-300 text-xs md:text-sm max-w-sm mx-auto leading-relaxed font-sans font-medium">Xin chào, {currentUser?.fullName}! Sẵn sàng nhập học kỳ đại học năm nhất dành cho cả PC & Thiết bị di động chưa?</p>
                  </div>
                  <div className="p-5 rounded-2xl bg-zinc-950/60 border border-zinc-800/50 text-left max-w-sm mx-auto">
                    ...
                    <p className="text-sm text-stone-200 font-sans font-medium leading-relaxed">Bạn đã chinh phục <span className="text-amber-400 font-black">{unlockedAchievements.length} / 6</span> cúp danh vọng. Hãy bắt đầu ca chơi mới để sưu tầm trọn bộ!</p>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-4 mt-6">
                    <button onClick={handleStartGame} className="w-full py-4.5 rounded-xl bg-amber-500 text-neutral-950 font-black text-sm shadow-lg hover:scale-[1.02] active:scale-95 transition-all cursor-pointer mx-auto block uppercase tracking-widest">
                      Bắt đầu nhập học
                    </button>
                    <button onClick={() => setPhase("LEADERBOARD")} className="w-full py-4.5 rounded-xl bg-zinc-900/80 hover:bg-zinc-800 border border-zinc-700 text-amber-400 font-black text-sm shadow-lg hover:scale-[1.02] active:scale-95 transition-all cursor-pointer mx-auto flex items-center justify-center gap-2 uppercase tracking-widest">
              ...
              {phase === "INTRO" && (
                <motion.div key="intro" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="max-w-4xl mx-auto p-8 rounded-3xl bg-[#0a0a0c]/80 backdrop-blur-2xl border border-zinc-800/60 shadow-2xl space-y-7">
                  <div className="text-center space-y-1.5 border-b border-zinc-800/50 pb-5">
                    ...
                    <h2 className="text-2xl md:text-3xl font-display font-black text-stone-100 uppercase tracking-wider">Thiết Lập Nhân Vật</h2>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2.5">
                      <label className="text-xs font-black text-zinc-300 uppercase tracking-widest block">Tên sinh viên:</label>
                      <div className="relative">
                        <span className="absolute inset-y-0 left-4 flex items-center text-zinc-400"><User className="h-5 w-5" /></span>
                        <input type="text" maxLength={24} value={playerName} onChange={(e) => setPlayerName(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-zinc-950/60 border border-zinc-700/50 rounded-xl text-sm font-black text-stone-100 focus:outline-none focus:border-amber-500/85 transition-all" placeholder="Tên của bạn..." />
                      </div>
                    </div>
                    <div className="space-y-2.5">
                      <label className="text-xs font-black text-zinc-300 uppercase tracking-widest block">Chuyên ngành đại học:</label>
                      <div className="space-y-2.5">
                        {[
                          ...
                          <button key={m.id} onClick={() => handleSelectMajor(m.id as any)} className={`w-full text-left p-4 rounded-xl border transition-all cursor-pointer flex items-center justify-between ${major === m.id ? "bg-amber-500/15 border-amber-500/50" : "bg-zinc-950/60 border-zinc-800/80 hover:border-zinc-700"}`}>
                            <div className="flex items-center gap-3.5">
                              <div className={`h-9 w-9 rounded-lg flex items-center justify-center text-neutral-900 ${major === m.id ? "bg-amber-400" : "bg-zinc-800 text-zinc-400"}`}>{m.icon}</div>
                              <div><p className="text-sm font-black text-stone-100">{m.name}</p><p className="text-[11px] text-stone-300 font-bold mt-0.5">{m.desc}</p></div>
                            </div>
                  ...
                  <div className="pt-5 border-t border-zinc-800/50 flex justify-end">
                    <button onClick={handleConfirmOnboarding} className="w-full md:w-auto px-10 py-4.5 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 text-neutral-950 font-black text-sm shadow-lg active:scale-98 transition-all flex items-center justify-center gap-2.5 cursor-pointer uppercase tracking-widest">
              ...
              {phase === "GAMEPLAY" && (
                <motion.div key="gameplay" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="space-y-6">
                  <div className="p-6 rounded-3xl bg-[#0a0a0c]/80 backdrop-blur-xl border border-zinc-800/60 shadow-2xl space-y-5">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-zinc-800/60 pb-3.5 gap-2">
                      <div>
                        <span className="text-xs font-black text-amber-500 uppercase tracking-widest font-mono">Tuần {dowInfo.week} — {dowInfo.label}</span>
                        <h2 className="text-2xl font-display font-black text-stone-100 mt-1">Hành Trình Ngày {currentDay} / {SEMESTER_DAYS}</h2>
                      </div>
                      <div className="flex items-center gap-2 bg-zinc-950/70 px-4 py-1.5 rounded-full border border-zinc-700/50">
                        {["morning", "afternoon", "evening"].map(s => {
                          const isActive = currentSlot === s;
                          const isDone = (s === "morning" && currentSlot !== "morning") || (s === "afternoon" && currentSlot === "evening");
                          return <span key={s} className={`h-3 w-3 rounded-full transition-all ${isActive ? "bg-amber-400 scale-110 shadow-[0_0_8px_rgba(245,158,11,0.5)]" : isDone ? "bg-zinc-600" : "bg-neutral-900 border border-zinc-700"}`} title={s === "morning" ? "Buổi Sáng" : s === "afternoon" ? "Buổi Chiều" : "Buổi Tối"} />
                        })}
                    ...
                    <div className="p-5 rounded-2xl bg-zinc-950/60 border border-zinc-800 flex items-start gap-4">
                      <div className={`h-10 w-10 rounded-lg flex items-center justify-center font-black text-lg shrink-0 ${slotSchedule.type === "class" ? "bg-amber-500/15 text-amber-400 border border-amber-500/30" : "bg-zinc-800/60 text-stone-300"}`}>
                    ...
                    <div className="space-y-4 pt-2">
                      <div className="flex items-center justify-between">
                        <label className="text-xs font-black text-zinc-300 uppercase tracking-widest block">Chọn hành động thực hiện:</label>
                        <span className="text-xs font-mono font-black text-zinc-400 uppercase">Click để chọn</span>
                      </div>
                      {slotSchedule.type === "class" ? (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
                          <motion.button whileHover={{ scale: 1.01, borderColor: "rgba(245, 158, 11, 0.4)" }} whileTap={{ scale: 0.98 }} onClick={() => handlePerformSlotActivity("lecture", "Học Chính Khóa", { gpa: 7, stress: 8, energy: -12, money: 0, happiness: -2 })} className="text-left p-5 rounded-2xl bg-gradient-to-br from-amber-500/10 to-zinc-950/60 hover:from-amber-500/20 hover:to-zinc-900/80 border border-zinc-800/80 hover:border-amber-500/50 transition-all cursor-pointer flex flex-col justify-between group min-h-40 md:h-44">
                            <div className="flex items-center gap-3"><div className="h-11 w-11 rounded-xl bg-zinc-900 border border-zinc-700 flex items-center justify-center text-amber-400 group-hover:scale-105 transition-transform shrink-0"><GraduationCap className="h-6 w-6" /></div><div><h5 className="text-sm font-black text-stone-100 uppercase tracking-wide">Lên Lớp Học</h5><span className="text-[10px] px-2 py-0.5 rounded bg-amber-500/15 border border-amber-500/30 text-amber-400 uppercase font-mono font-black mt-1 inline-block">Bắt buộc</span></div></div>
                            <div className="flex flex-wrap gap-1.5 mt-4"><span className="text-[11px] font-mono font-black px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-400">+7 GPA</span><span className="text-[11px] font-mono font-black px-2 py-0.5 rounded bg-rose-500/15 text-rose-400">-12 PIN</span><span className="text-[11px] font-mono font-black px-2 py-0.5 rounded bg-red-500/15 text-rose-300">+8 ÁP LỰC</span></div>
                          </motion.button>
                          <motion.button whileHover={{ scale: 1.01, borderColor: "rgba(251, 191, 36, 0.4)" }} whileTap={{ scale: 0.98 }} onClick={() => handlePerformSlotActivity("parttime", "Làm Thêm (Cúp học)", { gpa: -12, stress: 10, energy: -12, money: 15, happiness: -3 }, true)} className="text-left p-5 rounded-2xl bg-gradient-to-br from-rose-500/10 to-zinc-950/60 hover:from-rose-500/20 hover:to-zinc-900/80 border border-zinc-800/80 hover:border-rose-500/50 transition-all cursor-pointer flex flex-col justify-between group min-h-40 md:h-44">
                            <div className="flex items-center gap-3"><div className="h-11 w-11 rounded-xl bg-zinc-900 border border-zinc-700 flex items-center justify-center text-rose-400 group-hover:scale-105 transition-transform shrink-0"><Coins className="h-6 w-6" /></div><div><h5 className="text-sm font-black text-stone-100 uppercase tracking-wide">Cúp Làm Thêm</h5><span className="text-[10px] px-2 py-0.5 rounded bg-rose-500/15 border border-rose-500/30 text-rose-400 uppercase font-mono font-black mt-1 inline-block">Cúp học</span></div></div>
                            <div className="flex flex-wrap gap-1.5 mt-4"><span className="text-[11px] font-mono font-black px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-400">+15 VNĐ</span><span className="text-[11px] font-mono font-black px-2 py-0.5 rounded bg-rose-500/15 text-rose-400">-12 GPA</span><span className="text-[11px] font-mono font-black px-2 py-0.5 rounded bg-rose-500/15 text-rose-400">-12 PIN</span></div>
                          </motion.button>
                          <motion.button whileHover={{ scale: 1.01, borderColor: "rgba(129, 140, 248, 0.4)" }} whileTap={{ scale: 0.98 }} onClick={() => handlePerformSlotActivity("rest", "Ngủ Nướng (Cúp học)", { gpa: -15, stress: -10, energy: 20, money: 0, happiness: 8 }, true)} className="text-left p-5 rounded-2xl bg-zinc-950/60 hover:bg-zinc-900/80 border border-zinc-800/80 hover:border-indigo-500/50 transition-all cursor-pointer flex flex-col justify-between group min-h-40 md:h-44">
                            <div className="flex items-center gap-3"><div className="h-11 w-11 rounded-xl bg-zinc-900 border border-zinc-700 flex items-center justify-center text-indigo-400 group-hover:scale-105 transition-transform shrink-0"><Moon className="h-6 w-6" /></div><div><h5 className="text-sm font-black text-stone-100 uppercase tracking-wide">Cúp Ngủ Nướng</h5><span className="text-[10px] px-2 py-0.5 rounded bg-rose-500/15 border border-rose-500/30 text-rose-400 uppercase font-mono font-black mt-1 inline-block">Cúp học</span></div></div>
                            <div className="flex flex-wrap gap-1.5 mt-4"><span className="text-[11px] font-mono font-black px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-400">+20 PIN</span><span className="text-[11px] font-mono font-black px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-400">+8 VUI</span><span className="text-[11px] font-mono font-black px-2 py-0.5 rounded bg-rose-500/15 text-rose-400">-15 GPA</span></div>
                          </motion.button>
                        </div>
                      ) : (
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                          {[
                            { id: "library", name: "Thư Viện", base: { gpa: 10, stress: 12, energy: -14, money: 0, happiness: -4 }, icon: <BookOpen className="h-6 w-6" />, col: "text-teal-400", border: "hover:border-teal-500/40", bg: "hover:bg-teal-500/10", plus: "+10 GPA", minus: "-14 PIN" },
                            { id: "parttime", name: "Làm Thêm", base: { gpa: -2, stress: 14, energy: -18, money: 20, happiness: -5 }, icon: <Coins className="h-6 w-6" />, col: "text-amber-400", border: "hover:border-amber-500/40", bg: "hover:bg-amber-500/10", plus: "+20 VNĐ", minus: "-18 PIN" },
                            { id: "club", name: "Câu Lạc Bộ", base: { gpa: 0, stress: -6, energy: -8, money: -3, happiness: 12 }, icon: <Users className="h-6 w-6" />, col: "text-pink-400", border: "hover:border-pink-500/40", bg: "hover:bg-pink-500/10", plus: "+12 VUI", minus: "-6 ÁP LỰC" },
                            { id: "rest", name: "Nghỉ Ngơi", base: { gpa: 0, stress: -10, energy: 18, money: 0, happiness: 6 }, icon: <Moon className="h-6 w-6" />, col: "text-indigo-400", border: "hover:border-indigo-500/40", bg: "hover:bg-indigo-500/10", plus: "+18 PIN", minus: "-10 ÁP LỰC" },
                            { id: "party", name: "Nhậu Nhẹt", base: { gpa: -2, stress: -12, energy: -6, money: -8, happiness: 15 }, icon: <CupSoda className="h-6 w-6" />, col: "text-rose-400", border: "hover:border-rose-500/40", bg: "hover:bg-rose-500/10", plus: "+15 VUI", minus: "-12 ÁP LỰC" }
                          ].map(act => (
                            <motion.button key={act.id} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handlePerformSlotActivity(act.id, act.name, act.base)} className={`text-center p-4 rounded-2xl bg-zinc-950/60 border border-zinc-800/80 ${act.border} ${act.bg} transition-all cursor-pointer flex flex-col items-center justify-between min-h-[170px] md:h-48 group`}>
                              <div className={`h-12 w-12 rounded-xl bg-zinc-900 border border-zinc-700 flex items-center justify-center text-lg ${act.col} group-hover:scale-105 transition-transform shrink-0`}>{act.icon}</div>
                              <h5 className="text-xs md:text-sm font-black text-stone-100 uppercase tracking-wide mt-2">{act.name}</h5>
                              <div className="w-full flex flex-col gap-1 mt-3">
                                <span className="text-[11px] font-mono font-black py-0.5 rounded bg-emerald-500/15 text-emerald-400 block w-full text-center">{act.plus}</span>
                                <span className="text-[11px] font-mono font-black py-0.5 rounded bg-rose-500/15 text-rose-400 block w-full text-center">{act.minus}</span>
                              </div>
                            </motion.button>
                          ))}
                        </div>
                  ...
                  {showIntroTutorial && (
                    <div className="p-4.5 bg-amber-950/40 border border-amber-500/30 rounded-2xl relative text-xs md:text-sm text-amber-200 leading-relaxed font-sans flex items-start gap-2.5 animate-pulse backdrop-blur-sm">
                      <Info className="h-5 w-5 shrink-0 text-amber-400 mt-0.5" /><div className="flex-1"><span className="font-black text-stone-100 block mb-1 text-xs md:text-sm">Mẹo Sinh Tồn:</span>Hãy đảm bảo pin (Energy) của bạn luôn trên 20% và Stress dưới 80%. Hãy ngủ nướng nếu thấy pin cạn kiệt, và đi nhậu hoặc CLB để xả bớt Stress khi áp lực thi cử quá tải!</div><button onClick={() => setShowIntroTutorial(false)} className="text-zinc-400 hover:text-white shrink-0 cursor-pointer text-xs font-bold px-1.5">✕</button>
                    </div>
                  )}
                </motion.div>
              )}
              ...
              {phase === "DAILY_FEEDBACK" && dailyFeedback && (
                <motion.div key="daily_feedback" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="max-w-xl mx-auto p-8 rounded-3xl bg-[#0a0a0c]/80 backdrop-blur-2xl border border-zinc-800/60 shadow-2xl text-center space-y-6">
                  <div className="flex justify-center">
                    {dailyFeedback.isCritical === "success" && <span className="px-4 py-1.5 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-xs font-black uppercase tracking-widest">🔥 BẠO KÍCH CỰC ĐỈNH</span>}
                    {dailyFeedback.isCritical === "disaster" && <span className="px-4 py-1.5 rounded-full bg-red-500/15 border border-red-500/30 text-rose-400 text-xs font-black uppercase tracking-widest">⚠️ ĐẠI NẠN KHÓ ĐỠ</span>}
                    {dailyFeedback.isCritical === "none" && <span className="px-4 py-1.5 rounded-full bg-zinc-900/80 border border-zinc-700 text-stone-200 text-xs font-black uppercase tracking-widest">📝 NHẬT KÝ CUỐI NGÀY</span>}
                  </div>
                  <div className="space-y-1.5"><span className="text-[11px] font-mono text-zinc-300 uppercase font-black tracking-widest block">Chuỗi hoạt động: {dailyFeedback.activityName}</span><h3 className="text-xl md:text-2xl font-display font-black text-stone-100 leading-snug">{dailyFeedback.title}</h3></div>
                  <p className="text-sm text-stone-200 font-sans leading-relaxed text-left max-w-md mx-auto p-5 rounded-2xl bg-zinc-950/60 border border-zinc-800/80 shadow-inner font-bold">{dailyFeedback.text}</p>
                  {Object.keys(dailyFeedback.statsImpact).length > 0 && (
                    <div className="flex flex-wrap justify-center gap-2.5 text-xs font-black font-mono">
                      {Object.entries(dailyFeedback.statsImpact).map(([k, v]) => {
                        ...
                        return <span key={k} className={`px-3 py-1.5 rounded-lg ${isPos ? "bg-emerald-500/15 text-emerald-400" : "bg-rose-500/15 text-rose-400"}`}>{k.toUpperCase()}: {isPos ? `+${numV}` : numV}</span>;
                      })}
                    </div>
                  )}
                  {dailyFeedback.ancestralDodged && <div className="text-xs font-black text-emerald-400 bg-emerald-500/15 border border-emerald-500/30 px-4 py-1.5 rounded-full inline-flex items-center gap-1.5 mx-auto">🕉️ Tổ tiên hiển linh phù hộ: Triệt tiêu mọi thiệt hại chỉ số xấu!</div>}
                  <button onClick={handleConfirmDailyFeedback} className="w-full max-w-xs py-4 px-6 rounded-xl bg-amber-500 text-neutral-950 font-black text-sm shadow-lg hover:scale-[1.02] active:scale-95 transition-all cursor-pointer mx-auto block uppercase tracking-widest">Đồng ý gác ngày cũ</button>
                </motion.div>
              )}

              {/* PHASE 5: CAMPUS RANDOM EVENT SCREEN */}
              {phase === "EVENT" && activeEvent && (
                <motion.div key="event" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="max-w-xl mx-auto p-8 rounded-3xl bg-[#0a0a0c]/80 backdrop-blur-2xl border border-zinc-800/60 shadow-2xl space-y-7">
                  <div className="text-center space-y-2 border-b border-zinc-800/50 pb-4"><span className="text-xs font-black text-indigo-400 bg-indigo-500/15 border border-indigo-500/30 px-4 py-1.5 rounded-full uppercase tracking-widest inline-block">⚠️ BIẾN CỐ PHÁT SINH</span><h3 className="text-xl md:text-2xl font-display font-black text-stone-100 leading-snug pt-1">{activeEvent.title}</h3></div>
                  <p className="text-sm text-stone-200 font-sans font-bold leading-relaxed p-5 rounded-2xl bg-zinc-950/50 border border-zinc-800/60 text-left">{activeEvent.description}</p>
                  <div className="space-y-3">
                    {activeEvent.options.map(opt => {
                      const isSelected = choiceFeedback?.text === opt.outcome.textFeedback;
                      return (
                        <button key={opt.id} disabled={choiceFeedback !== null} onClick={() => handleSelectEventOption(opt)} className={`w-full text-left p-5 rounded-2xl border transition-all cursor-pointer block ${isSelected ? "bg-amber-500/15 border-amber-500/50 text-amber-200" : choiceFeedback !== null ? "bg-zinc-950/40 border-zinc-800 opacity-50 cursor-not-allowed" : "bg-zinc-900/60 border-zinc-700/60 hover:border-zinc-600"}`}>
                          <div className="flex items-center justify-between gap-1"><span className="text-sm md:text-base font-black leading-snug">{opt.text}</span>{isSelected && <CheckCircle2 className="h-5 w-5 text-amber-400 shrink-0 ml-2" />}</div>
                        </button>
                      );
                    })}
                  </div>
                  <AnimatePresence>
                    {choiceFeedback && (
                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-5 rounded-2xl bg-zinc-950/60 border border-zinc-700/80 text-left space-y-4 shadow-inner">
                        ...
                        <div className="flex flex-wrap gap-2 text-[11px] font-mono font-black">
                          {Object.entries(choiceFeedback.statsImpact).map(([k, v]) => {
                            ...
                            return <span key={k} className={`px-2.5 py-1 rounded-lg ${isP ? "bg-emerald-500/15 text-emerald-400" : "bg-rose-500/15 text-rose-400"}`}>{k.toUpperCase()}: {isP ? `+${numV}` : numV}</span>;
                          })}
                        </div>
              ...
              {phase === "SUMMARY" && (
                <motion.div key="summary" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="max-w-xl mx-auto p-8 md:p-10 rounded-3xl bg-[#0a0a0c]/80 backdrop-blur-2xl border border-zinc-800/60 shadow-2xl space-y-7 text-center">
                  <div className="relative inline-block"><span className="absolute inset-0 rounded-full bg-emerald-500/15 blur-xl animate-pulse"></span><div className="h-20 w-20 rounded-full bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center text-5xl">🎉</div></div>
                  <div className="space-y-2"><h2 className="text-3xl font-display font-black text-stone-100 uppercase tracking-widest">Hoàn Thành Học Kỳ!</h2><p className="text-sm font-bold text-stone-300 max-w-sm mx-auto leading-relaxed">Chúc mừng bạn đã sinh tồn xuất sắc qua 14 ngày đầy bão tố! Kết quả đã lưu vào CSDL.</p></div>
                  <div className={`p-6 rounded-2xl border backdrop-blur-sm ${badge.color} text-left space-y-2.5 max-w-md mx-auto shadow-md`}><span className="text-[11px] font-black uppercase tracking-widest block opacity-75">BẰNG CHỨNG NHẬN ĐẠI HỌC</span><h4 className="text-base md:text-lg font-display font-black tracking-wide leading-tight">{badge.title}</h4><p className="text-xs md:text-sm text-stone-200 leading-relaxed font-sans font-bold">{badge.description}</p></div>
                  
                  {/* Điểm số */}
                  <div className="p-6 rounded-2xl bg-gradient-to-br from-indigo-500/15 to-purple-500/15 border border-indigo-500/40 max-w-md mx-auto backdrop-blur-sm">
                    <span className="text-xs text-indigo-300 font-black uppercase tracking-widest block mb-2">ĐIỂM XẾP HẠNG CỦA BẠN</span>
                  ...
                  <div className="grid grid-cols-2 gap-4 max-w-md mx-auto pt-2">
                    <button onClick={() => setPhase("LEADERBOARD")} className="w-full py-4.5 rounded-xl bg-amber-500 text-neutral-950 font-black text-sm shadow-lg hover:scale-102 active:scale-95 transition-all cursor-pointer uppercase tracking-widest">
                      Bảng Xếp Hạng
                    </button>
                    <button onClick={handleRestart} className="w-full py-4.5 rounded-xl bg-zinc-900/80 border border-zinc-700 text-stone-200 font-black text-sm shadow-lg hover:scale-102 active:scale-95 transition-all cursor-pointer uppercase tracking-widest">
                      Chơi Lại
                    </button>
                  </div>
                </motion.div>
              ...
              {phase === "GAMEOVER" && (
                <motion.div key="gameover" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="max-w-xl mx-auto p-8 rounded-3xl bg-[#0a0a0c]/80 backdrop-blur-2xl border border-zinc-800/60 shadow-2xl space-y-7 text-center animate-pulse">
                  <div className="relative inline-block"><span className="absolute inset-0 rounded-full bg-rose-500/15 blur-xl"></span><div className="h-20 w-20 rounded-full bg-rose-500/15 border border-rose-500/30 flex items-center justify-center text-5xl animate-bounce">💀</div></div>
                  <div className="space-y-2"><h2 className="text-3xl font-display font-black text-rose-500 uppercase tracking-widest">Kỳ Thi Thất Bại!</h2><p className="text-sm font-bold text-stone-300">Năm nhất đại học quả là một đấu trường đầy chông gai...</p></div>
                  <div className="p-6 rounded-2xl bg-rose-950/40 backdrop-blur-sm border border-rose-900/50 text-left space-y-2.5 max-w-md mx-auto shadow-md"><span className="text-[11px] font-black text-rose-400 uppercase tracking-widest block opacity-75">NGUYÊN NHÂN BI KỊCH</span><p className="text-sm text-stone-200 font-sans leading-relaxed font-bold">{deathCause}</p></div>
                  <button onClick={handleRestart} className="w-full max-w-xs py-4.5 rounded-xl bg-gradient-to-r from-red-500 to-rose-600 text-white font-black text-sm shadow-lg hover:scale-[1.02] active:scale-95 transition-all cursor-pointer mx-auto block uppercase tracking-widest">
      ...
      <footer className="border-t border-zinc-800/50 py-6 px-4 text-center text-xs text-zinc-400 font-mono select-none relative z-10">
      ...
      <AnimatePresence>
        {showHistoryModal && (
          <>
            <motion.div key="modal_backdrop" initial={{ opacity: 0 }} animate={{ opacity: 0.6 }} exit={{ opacity: 0 }} onClick={() => setShowHistoryModal(false)} className="fixed inset-0 bg-black/85 z-[90] backdrop-blur-sm" />
            <motion.div key="modal_panel" initial={{ opacity: 0, y: 30, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.98 }} className="fixed inset-x-4 top-10 bottom-10 md:inset-x-auto md:left-1/2 md:-translate-x-1/2 md:w-[520px] md:h-[620px] bg-[#0a0a0c]/90 backdrop-blur-2xl border border-zinc-800/60 rounded-3xl z-[100] flex flex-col p-6 md:p-8 shadow-2xl overflow-hidden">
              <div className="flex items-center justify-between border-b border-zinc-800/60 pb-4 mb-4 shrink-0">
                <div className="flex items-center gap-2"><FileText className="h-5 w-5 text-amber-500" /><h4 className="font-display font-black text-sm uppercase tracking-wider">Lịch Sử Sinh Tồn ({history.length} Ngày)</h4></div>
                <button onClick={() => setShowHistoryModal(false)} className="text-xs text-stone-300 hover:text-white bg-zinc-900/80 hover:bg-zinc-800 px-4 py-2 rounded-xl cursor-pointer transition font-bold border border-zinc-700">Đóng</button>
              </div>
              <div className="flex-1 overflow-y-auto space-y-4 pr-1 pb-4 scrollbar-none">
                {history.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center text-zinc-500 py-12"><BookmarkCheck className="h-12 w-12 text-zinc-700 mb-3 stroke-[1.5]" /><p className="text-sm font-black text-zinc-400">Chưa có nhật ký ghi nhận.</p><p className="text-xs text-zinc-500 mt-1">Hoàn tất ngày chơi đầu tiên để tạo tệp lưu trữ.</p></div>
                ) : (
                  history.map((entry, idx) => (
                    <div key={idx} className="p-4 rounded-xl bg-zinc-950/60 border border-zinc-800/60 text-left space-y-2">
                      <div className="flex items-center justify-between text-xs font-mono"><span className="text-amber-400 font-black uppercase">Ngày thứ {entry.day}</span>{entry.eventTitle && <span className="px-2.5 py-0.5 rounded bg-indigo-500/15 border border-indigo-500/30 text-indigo-400 text-[10px] font-black uppercase">Biến cố</span>}</div>
                      <p className="text-sm font-bold text-stone-100">{entry.activityName}</p>
                      {entry.eventTitle && <p className="text-xs text-zinc-400 mt-1 italic font-medium">Hành xử biến cố: {entry.eventTitle}</p>}
                      <div className="mt-2.5 flex flex-wrap gap-1.5 text-[11px] font-mono font-black">
                        {Object.entries(entry.statsImpact).map(([k, v]) => {
                          ...
                          return <span key={k} className={`px-2 py-0.5 rounded ${isPos ? "bg-emerald-500/15 text-emerald-400" : "bg-rose-500/15 text-rose-400"}`}>{k.toUpperCase()}: {isPos ? `+${numVal}` : numVal}</span>;
                        })}
                      </div>
                    </div>
                  ))
